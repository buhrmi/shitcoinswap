...
class WithdrawalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create withdrawal" do
    assert_difference('Withdrawal.count') do
      post withdrawals_url, params: { withdrawal: { amount: @withdrawal.amount, error: @withdrawal.error, error_at: @withdrawal.error_at, receiver_address: @withdrawal.receiver_address, signed_transaction: @withdrawal.signed_transaction, status: @withdrawal.status, submitted_at: @withdrawal.submitted_at, symbol: @withdrawal.symbol, transaction_id: @withdrawal.transaction_id, tries: @withdrawal.tries, user_id: @withdrawal.user_id } }
    end
