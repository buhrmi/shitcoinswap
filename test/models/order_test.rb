...
class OrderTest < ActiveSupport::TestCase
  ...
  test 'can not submit buy-limit order with insufficient balance' do
    available_jpy = alice.available_balance(jpy)
    assert_raises ActiveRecord::RecordInvalid do
      Order.create!(user: alice, price: 1.1, kind: 'limit', side: 'buy', quantity: available_jpy, base_asset: eth, quote_asset: jpy)
    end
  end

  test 'can submit buy-limit order with sufficient balance' do
    available_jpy = alice.available_balance(jpy)
    Order.create!(user: alice, price: 1, kind: 'limit', side: 'buy', quantity: available_jpy, base_asset: eth, quote_asset: jpy)
    assert_equal 0, alice.available_balance(jpy)
  end
  ...
  test 'can not submit sell-limit order with insufficient balance' do
    ...
    assert_raises ActiveRecord::RecordInvalid do
      Order.create!(user: alice, price: 1, kind: 'limit', side: 'sell', quantity: available_eth + 1, base_asset: eth, quote_asset: jpy)
    end
  end

  test 'can submit sell-limit order with sufficient balance' do
    available_eth = alice.available_balance(eth)
    Order.create!(user: alice, price: 1, kind: 'limit', side: 'sell', quantity: available_eth, base_asset: eth, quote_asset: jpy)
  ...
  test "buy and sell limit orders fill each other" do
    ...
    order1 = alice.orders.create!(side: 'buy', kind: 'limit', quantity: 3, price: 5, base_asset: eth, quote_asset: jpy)
    order2 = bob.orders.create!(side: 'sell', kind: 'limit', quantity: 3, price: 5, base_asset: eth, quote_asset: jpy)
    order1.process!
  ...
  test "buy and sell limit orders fill each other partially" do
    ...
    order1 = alice.orders.create!(side: 'buy', kind: 'limit', quantity: 7, price: 6.5, base_asset: eth, quote_asset: jpy)
    order2 = bob.orders.create!(side: 'sell', kind: 'limit', quantity: 3, price: 5, base_asset: eth, quote_asset: jpy)
    order2 = bob.orders.create!(side: 'sell', kind: 'limit', quantity: 3, price: 6, base_asset: eth, quote_asset: jpy)
    order2 = bob.orders.create!(side: 'sell', kind: 'limit', quantity: 3, price: 7, base_asset: eth, quote_asset: jpy)
  ...
  test "sell limit and buy market orders fill each other partially" do
    ...
    order1 = alice.orders.create!(side: 'sell', kind: 'limit', quantity: 7, price: 2, base_asset: eth, quote_asset: jpy)
  ...
  test "no rounding hickups on buy market vs sell limit" do
    ...
    order1 = alice.orders.create!(side: 'sell', kind: 'limit', quantity: 1, price: 0.3, base_asset: eth, quote_asset: jpy)
  ...
  test "buy limit and sell market orders fill each other" do
    ...
    order1 = alice.orders.create!(side: 'buy', kind: 'limit', quantity: 3, price: 2, base_asset: eth, quote_asset: jpy)
  ...
  test "sell everything at limit and buy from myself" do
    ...
    order1 = alice.orders.create!(side: 'sell', kind: 'limit', quantity: alice_eth_before, price: 1, base_asset: eth, quote_asset: jpy)
